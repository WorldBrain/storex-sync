...
describe('CustomAutoPkMiddleware', () => {
    ...
    it('should be able to set custom auto PKs on complex createObject operations', async () => {
        let counter = 0
        const { storageManager } = await setupTest({
            pkGenerator: () => `some-pk-${++counter}`,
        })
        const { object: user } = await storageManager
            .collection('user')
            .createObject({
                displayName: 'Joe',
                emails: [{ address: 'foo@bla.com' }],
            })
        expect(user).toEqual({
            id: 'some-pk-1',
            displayName: 'Joe',
        ...
        expect(
            await storageManager
                .collection('user')
                .findOneObject({ id: user.id }),
        ).toEqual({ id: user.id, displayName: 'Joe' })
        expect(
            await storageManager
                .collection('email')
                .findOneObject({ id: email.id }),
        ).toEqual({ id: email.id, user: user.id, address: 'foo@bla.com' })
    })
